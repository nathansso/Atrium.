...
export async function launchCurriculum(
  ...
  if (record.launch) {
    return {
      ...
      assignment: assignmentFromCurriculum(record.draft, record.launch.teaching_intent),
      run_id: record.launch.run_id,
      reused: true,
    };
  }

  const assignment = assignmentFromCurriculum(record.draft, request.teaching_intent);
  ...
  return { launch, assignment, run_id: outcome.state.run_id, reused: false };
}
